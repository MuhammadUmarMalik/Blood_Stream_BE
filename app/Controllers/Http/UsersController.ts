...
export default class UserController {
  ...
  public async getUsersWithinRadius({
    response,
    selectedHospital,
    blood_group,
  }) {
    try {
      ...
      if (!nominatimResponse.data || nominatimResponse.data.length === 0) {
        return response
          .status(404)
          .send({ error: "Hospital location not found." });
      }
      ...
      const boundary = `ST_Buffer(ST_GeomFromText('POINT(${lon} ${lat})'), ${
        radius * 1000
      })`;
      ...
      if (users.length === 0) {
        return response
          .status(404)
          .send({ message: "No users found within 3km radius." });
      }
      ...
      return response
        .status(500)
        .send({ error: "An error occurred while searching for users." });
    }
  }

  public async show({ request, response }: HttpContextContract) {
    const { selectedHospital, blood_group } = request.qs();
    await this.getUsersWithinRadius({
      response,
      selectedHospital,
      blood_group,
    });
